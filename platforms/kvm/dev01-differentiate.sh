...
export ERPNEXT_SITE="dev01"
export ERPNEXT_DNS="dev01"
...
if sudo test -d "$BENCH_DIR_ORIG" && ! sudo test -L "$BENCH_DIR"; then
    ...
    echo "  [OK] symlinked frappe-bench -> frappe-bench-D1IRBL (venv paths preserved)"
elif sudo test -L "$BENCH_DIR"; then
    echo "  [OK] frappe-bench-D1IRBL symlink already exists — skipping"
else
...
upstream frappe-frappe-bench-D1IRBL-dev01.iridium.blue {
    server 127.0.0.1:8000;
}
upstream frappe-socketio-frappe-bench-D1IRBL {
    server 127.0.0.1:9000;
}

server {
    listen 80;
    server_name dev01.iridium.blue;
...
server {
    ...
    location /socket.io {
        ...
        proxy_pass http://frappe-socketio-frappe-bench-D1IRBL;
    }
...
NGINXEOF
echo "  [OK] /etc/nginx/sites-available/dev01.iridium.blue"
