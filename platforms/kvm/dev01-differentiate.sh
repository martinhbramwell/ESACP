...
sudo -u "$ERP_USER" bash -c "cd $BENCH_DIR && bench --site $SITE_URL migrate" 2>&1   | grep -E "^(Migrating|Executing|Updating|Building)" | tail -10
...
sudo cp /tmp/rendered/nginx_vhost.conf /etc/nginx/sites-available/dev01.iridium.blue
