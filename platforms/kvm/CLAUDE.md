...
- **E**: place ddlViews.sql; **E1**: `g1_seed_patch_log.py` — seeds `tabPatch Log` for patches that crash on restored production DBs (e.g. `delete_duplicate_indexes` queries missing `session_status` table). Called twice: before F (protects F's migrate) and as G1 after G (restore wipes DB, protects G2's migrate).
- **F**: installApps.sh
- **G-pre**: strip `DEFINER=<user>` from backup SQL via `gpre_strip_definer.py` (Python `re.sub` — no sed)
- **G**: handleRestore.sh (restore → migrate → views)
- **G1**: re-seed `tabPatch Log` — same script as E1, re-run because `bench restore` in G dropped and recreated the DB
