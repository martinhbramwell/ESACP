...
BENCH_DIR="/home/erpadm/frappe-bench-D3IRBL"
SITE_URL="dev03.iridium.blue"
ERP_USER="erpadm"
MYPWD="erpnext_build"
ERP_USER_PWD="sasa"
...
if sudo test -d "$BENCH_DIR_ORIG" && ! sudo test -L "$BENCH_DIR"; then
    ...
    echo "  [OK] symlinked frappe-bench -> frappe-bench-D3IRBL (venv paths preserved)"
elif sudo test -L "$BENCH_DIR"; then
    echo "  [OK] frappe-bench-D3IRBL symlink already exists — skipping"
else
...
sudo -u "$ERP_USER" cp /tmp/ddlViews.sql "/home/erpadm/frappe-bench-D3IRBL/sites/dev03.iridium.blue/private/files/ddlViews.sql"
...
sudo cp /tmp/rendered/nginx_vhost.conf /etc/nginx/sites-available/dev03.iridium.blue
echo "  [OK] /etc/nginx/sites-available/dev03.iridium.blue"
...
sudo ln -sf /etc/nginx/sites-available/dev03.iridium.blue /etc/nginx/sites-enabled/dev03.iridium.blue
