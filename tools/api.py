...
def _resolve_power_target(hostname: str) -> tuple[dict, str]:
...
def vm_start(hostname: str):
    ...
    _, hypervisor = _resolve_power_target(hostname)
...
def vm_stop(hostname: str):
    ...
    host_cfg, hypervisor = _resolve_power_target(hostname)
    is_hub = host_cfg.get("wg_role") == "hub"
    try:
        msg = vm_power.stop(hypervisor, hostname, lambda _: None, is_hub=is_hub)
