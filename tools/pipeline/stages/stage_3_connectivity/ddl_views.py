...
def _ddl_present_on_vm(config: Config) -> bool:
    r = ssh_run(config, "test -f /tmp/ddlViews.sql && echo y", timeout=10)
    return r.returncode == 0 and "y" in r.stdout


def ensure_ddl_views(config: Config, emit: Emit) -> TaskResult:
    """SCP views.ddl → /tmp/ddlViews.sql on the VM."""
    if _ddl_present_on_vm(config):
        return TaskResult(True, False, "ddlViews.sql already on VM")

    if not VIEWS_DDL_SRC.exists():
        return TaskResult(True, False,
                          f"ddlViews.sql not found at {VIEWS_DDL_SRC} — skipping")

    r = scp_to_vm(config, [str(VIEWS_DDL_SRC)], "/tmp/ddlViews.sql", timeout=30)
    if r.returncode != 0:
