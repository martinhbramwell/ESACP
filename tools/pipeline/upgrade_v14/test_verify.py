#!/usr/bin/env python3
"""verify_upgrade_v14 returns 3 checks; all_passed True on green."""
...
def test_verify_returns_three_checks_all_passing_on_green() -> bool:
    def fake(_cfg, _cmd, *, timeout=30):
        return ok("version-14\nY\n")

    orig = patch_ssh(verify, fake)
    try:
        results = verify.verify_upgrade_v14(make_config())
    finally:
        verify.ssh_run = orig
    if len(results) != 3:
    ...
    if not verify.all_passed(results):
        ...
        return False
    print("PASS: verify returns 3 checks; all_passed True on green path")
    return True
