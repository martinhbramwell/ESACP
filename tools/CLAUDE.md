...
- `applyV16PostMigrateFixups` (#480 umbrella): runs catalogued V13→V16 post-migrate fix/probe scripts on a lab VM. Catalogue (the `FIXUPS` tuple in `v16_post_migrate_fixups.py` — one `Fixup(label, script, expected, changed_marker)` row per child, all run via the same bench-venv invocation): R1 = recreate `Web Page route='home'` from runtime-salvaged `tabSingles` Homepage singleton (`r1_recreate_web_page_home.py`, #486); R3 = disable orphan `IRS 1099 Form` Print Format (`r3_disable_irs_1099_pf.py`, #498); R8 = end-to-end naming-series acceptance probe — creates a real Sales Invoice on the tenant test series and asserts the incremented name (`r8_naming_series_probe.py`, #617). Dispatcher calls `tools/pipeline/orchestration/v16_post_migrate_fixups.py`. Per-script invocation + probe-parsing is factored into `fix_script_runner.run_fix_script()`. Add a new #480 child by appending one `FIXUPS` row — no per-script function needed. NB requires `server_script_enabled` in common_site_config (#626) so document-creating probes run the tenant's commission server scripts.
