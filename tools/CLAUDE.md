...
- Step 10 generates a site-specific `ce_sri_parms.json` from the production base (`~/projects/Logichem/ce_sri/example_srvr_files/ce_sri_parms.json`) with per-VM overrides: `local_site`, `api_protocol=https`, `api_port=443`, `certificate_location`, `local_site_nickname`, `company_logo_location`, `test_or_production_mode=1`. H4e then injects the fresh API key (via `h4e_patch_parms.py`) and `UPDATE_SRI_SERVICE_PARAMETERS.py` generates all `.env` variants — no sed
