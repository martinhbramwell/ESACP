...
**Introspection sidebar (housekeeping, periodic)** — a sidebar session devoted to clearing accumulated carry-forward operator-reminders, fixing recurring drift caught only at verdict-time, and structurally reorganizing memory/process docs that have grown organically. Not aimed at any product feature; aimed at the system that builds product features.

- **Trigger (cadence)** — every 5–7 sessions, OR when carry-forward operator-reminders cross 3+ unresolved items, whichever comes first. The threshold is rough; operator may invoke earlier when the reminder list visibly grows session-over-session without attrition.
- **Mechanical sidebar trigger (diff-based)** — independent of cadence, a session is **definitively** an introspection-sidebar (and not a housekeeping-bundle) if its diff does any of: (a) edits to `MEMORY.md` indexing — adding, removing, or restructuring entries; OR (b) attrition of carry-forward operator-reminders from the next-session agenda — dropping items that have aged out or been terminally homed. If either is touched the session **is** a sidebar even when other housekeeping issues close in the same bundle. The diff is authoritative; tag the close commit accordingly. (ESACP#400 Stage 5 CM-4=A / S66.)
- **Allowed scope** — documentation scrubs (CLAUDE.md, RUNBOOK, agendas, minutes), wording/terminology fixes, structural reorganization of MEMORY.md or other auto-loaded docs, filing tracker issues for observations recycled in carry-forward reminders for ≥2 sessions, dropping resolved or terminally-homed items from carry-forward.
- **Discipline** — same housekeeping-bundle guardrails: each substantive change still its own GitHub issue, each issue still closed by `fixes #N` in its own commit. No mixing with substantive code work; if a substantive bug is discovered mid-sidebar, file it and defer to a dedicated 1:1:1 session.
- **Out of scope** — anything touching pipeline code, dispatchers, SUT scripts, Ansible, or SOPS-backed runtime config. Those follow 1:1:1 discipline regardless of how convenient bundling looks.
...
- **Naming** — `umbrella/<short-topic>` (e.g. `umbrella/v16-upgrade`, `umbrella/matrix-run-08`). Single prefix so `git branch --list 'umbrella/*'` enumerates live umbrellas. The `umbrella/*` prefix is **reserved** for branches that actually have sub-branches OR are long-lived across sessions; single-issue / single-session multi-commit work uses `feat/{issue#}-…` or `docs/{issue#}-…` even when the work is doc-flavoured (ESACP#400 Stage 5 DI-2 / S66).
