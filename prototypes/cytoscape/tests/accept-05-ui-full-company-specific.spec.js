...
test.describe('Acceptance Run 05 — UI full company-specific ERPNext', () => {
  ...
  test('self-check + UI destroy + drag-deploy dev01 — job completes, UI converges, ERPNext canary reachable', async ({ page, browser }) => {
    ...
    if (existing && existing.provisioned) {
      console.log(`[accept-05] ${params.target_vm} present and provisioned — destroying via UI`)
      ...
      console.log(`[accept-05] ${params.target_vm} pre-registered only — CLI destroy to unregister`)
